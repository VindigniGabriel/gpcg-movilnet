<template>
  <div class="Chart">
    <v-subheader>Requerimientos por status At. presencial</v-subheader>
...
export default {
  ...
  computed: {
    ...
    requests: state => state.getRequestsOptions.filter(o => o.face === true),
    requestsFilter: state =>
      state.getRequestsOptions
        .filter(o => o.face === true && state.options.includes(o.name))
        .map(r => r.name)
  },
  watch: {
    getRequestsLocation() {
  ...
  methods: {
    ...
    fillData() {
      ...
      let labels = this.getStatusOptions
        .filter(s => s.face === true || s.face === "n/a")
        .map(r => r.name);
      let requests = null;
